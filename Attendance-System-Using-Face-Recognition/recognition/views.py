...
#Function for updating attendance in database for student in
...
#Function for updating attendance in database for student out
...
# Function for checking validating time in out
...
# Function for converting hour to minutes
...
# Function for managing hours and date in attendance report
def hours_vs_date_given_student(present_qs, time_qs, admin=True):
    ...
    if(admin):
        ...
        plt.savefig(
            './recognition/static/recognition/img/attendance_graphs/student_login/1.png')
        plt.close()
    return qs


# Function for hours and student given date
def hours_vs_student_given_date(present_qs, time_qs):
    ...
    plt.tight_layout()
    plt.savefig(
        './recognition/static/recognition/img/attendance_graphs/hours_vs_student/1.png')
    plt.close()
    return qs

#Function for counting total number of student with out admin in registered 
def total_number_students():
    qs = User.objects.all()
    return (len(qs) - 1)
    # -1 to account for admin

#Function for today present student
...
# Function for This week student count vs date
def this_week_stdnt_count_vs_date():
    today = datetime.date.today()
...
# Function for last week student count vs date
def last_week_stdnt_count_vs_date():
    today = datetime.date.today()
    some_day_last_week = today-datetime.timedelta(days=7)
    monday_of_last_week = some_day_last_week - \
        datetime.timedelta(days=(some_day_last_week.isocalendar()[2] - 1))
...
def add_photos(request):
    ...
    if request.method == 'POST':
        ...
        if username_present(username):
            ...
            messages.warning(
                request, f'No such username found. Please register student first.')
...
# Function for mark your attendance in
...
# Function for mark your attendance out
...
#Calling the total no of students and student present today in this function
@login_required
def view_attendance_home(request):
    total_num_of_emp = total_number_students()
    emp_present_today = students_present_today()
    this_week_stdnt_count_vs_date()
    last_week_stdnt_count_vs_date()
...
#Function for View students addendance by date
@login_required
...
#Function for View students attencance
@login_required
        ...
        if form.is_valid():
            ...
            if username_present(username):
                ...
                if date_to < date_from:
                    ...
                    if (len(time_qs) > 0 or len(present_qs) > 0):
                        qs = hours_vs_date_given_student(
...
#Function for view my attendance student login
@login_required
